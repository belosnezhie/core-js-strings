...
function removeFirstOccurrences(str, value) {
  ...
  const result = str.slice(0, index) + str.slice(index + length, str.length);
  return result;
}
...
function formatTime(minutes, seconds) {
  const first = minutes.toString();
  const minutesInStr = first.padStart(2, 0);
  const second = seconds.toString();
  const secondsInStr = second.padStart(2, 0);
  return minutesInStr.concat(`:${secondsInStr}`);
}
...
function countVowels(str) {
  ...
  const map = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  const arr = str.split('');
  const result = arr.reduce((accumulator, currentValue) => {
    for (let i = 0; i < map.length; i += 1) {
      if (currentValue === map[i]) {
...
function isPalindrome(str) {
  ...
  if (replacedStr.length % 2 !== 0) {
    const middleIndex = (replacedStr.length - 1) / 2;
    for (let i = 1; i <= middleIndex; i += 1) {
      if (lowerCaseStr[middleIndex - i] !== lowerCaseStr[middleIndex + i]) {
        return false;
      }
    }
    return true;
  }
  const middleIndexRigt = replacedStr.length / 2;
  for (let i = 0; i <= middleIndexRigt; i += 1) {
    if (
      lowerCaseStr[middleIndexRigt - 1 - i] !==
      lowerCaseStr[middleIndexRigt + i]
    ) {
      return false;
    }
  }
...
function reverseWords(str) {
  let newStr = '';
  const arr = str.split(' ');
  const reversedArr = arr.map((currentValue) => {
    newStr = '';
    for (let i = 1; i <= currentValue.length; i += 1) {
      const newChar = currentValue.length - i;
      newStr += currentValue[newChar];
    }
    return newStr;
  });
...
function invertCase(str) {
  const arr = str.split('');
  const newArr = arr.map((currentValue) => {
...
function extractNameFromTemplate(value) {
  const indexOfExclamationPoint = value.indexOf('!');
  return value.substring(7, indexOfExclamationPoint);
}
...
function encodeToRot13(str) {
  const normalMap = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const codeMap = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let newStr = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === ' ' || str[i] === '?' || str[i] === '!') {
      newStr += str[i];
    } else {
      const index = normalMap.indexOf(str[i]);
      newStr += codeMap[index];
    }
  }
  return newStr;
  // let newStr = '';
  ...
  //   if (str[i] === 'm' || str[i] === 'M') {
  //     newStr += String.fromCharCode(number - 13);
  //   } else {
  //     newStr += String.fromCharCode(number + 13);
  //   }
  // }
